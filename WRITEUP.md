...
I never read the problem set in detail. I don't fully understand the winning strategy. I couldn't explain the arb-risk-weighted sizing formula or why the 5% floor exploit works. The entire solution — from first strategy to final submission — was generated by AI agents. I just pointed them at the problem, said "keep going," and collected the results.

This is exactly what [Rich Sutton's Bitter Lesson](http://www.incompleteideas.net/IncsightBlurb/SRblurb.html) predicts: general methods that leverage massive computation will always beat human-crafted domain expertise. I brought zero quant finance knowledge. The agents brought 1,039 strategies, 2,000+ evaluations, and the willingness to try everything. Scale won.

## Result

**1st Place** on the Paradigm Optimization Arena Prediction Market Challenge (April 9, 2026 Hackathon).

All qualifying submissions were rescored with a fresh random seed for final standings:
...
- **Agent Tasks**: 980+ parallel agent runs
...
3. **Size based on arb risk.** `size = k × retail_mult × (1 - arb_prob × damping)`. Quote big when the arb is unlikely to hit you (extreme prices, wide spreads), small when it's dangerous (near 50%, tight spreads).
